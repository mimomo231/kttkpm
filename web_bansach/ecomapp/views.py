...
class HomeView(EcomMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        allcategories = list(Category.objects.all())
        # print(allcategories.first
        
        l = [i.title for i in allcategories]
        self.request.session['l'] = l
        if page_number == None:
            page_number = 1

        r = int(int(page_number)/10)*10
        lp = [i for i in range(r+1,r+11)]
        product_list = paginator.get_page(page_number)
        # print(product_list.number)
        context = {
...
def product_in_cat(request,cat_slug):
        c = get_object_or_404(Category, slug=cat_slug)
        products = c.book_set.all().order_by("-id")
        ...
        template = loader.get_template('allproducts.html')
